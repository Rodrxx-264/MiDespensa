...
export function ItemProducto({ producto, onUpdate, onDelete }: { producto: Producto; onUpdate: (id: string, p: Partial<Producto>) => Promise<void>; onDelete: (id: string) => Promise<void> }) {
  ...
  return <article className={`rounded-[22px] border border-[var(--line)] bg-[var(--surface)] p-3 shadow-sm md:rounded-3xl md:p-4 ${producto.estado === "comprado" || producto.status === "not_found" ? "line-through-soft" : ""}`}><div className="grid gap-3"><div><div className="flex items-start justify-between gap-2"><h3 className="text-lg font-black leading-tight tracking-[-.03em]">{producto.substitute_name || producto.nombre}</h3><span className="rounded-[10px] bg-[var(--line)] px-2 py-1 text-[11px] font-black text-[var(--muted)]">{producto.status === "not_found" ? "No encontrado" : priorityLabel(producto.priority)}</span></div><p className="mt-1 text-sm text-[var(--muted)]">{producto.cantidad} {producto.unidad} · {producto.agregado_por_nombre ?? "Sin autor"}</p><p className="mt-1 text-sm font-semibold text-[var(--muted)]">{producto.precio_estimado ? formatoQ.format(producto.precio_estimado) : "Sin precio"} {producto.tienda_sugerida ? `en ${producto.tienda_sugerida}` : ""}</p>{producto.substitute_name && <p className="mt-1 text-xs text-[var(--muted)]">Original: {producto.nombre}</p>}</div><div className="grid grid-cols-[1fr_auto_auto] gap-2"><Button aria-label={`Marcar ${producto.nombre} como comprado`} variant="secondary" onClick={() => setOpen(true)}><span className="inline-flex items-center justify-center gap-2"><Check aria-hidden="true" size={18}/> Comprar</span></Button><AgregarProducto producto={producto} onGuardar={(p) => onUpdate(producto.id, p)}/><Button aria-label={`Eliminar ${producto.nombre}`} variant="danger" onClick={() => onDelete(producto.id)}><Trash2 aria-hidden="true" size={18}/></Button></div></div><Modal open={open} onOpenChange={setOpen} title="Marcar como comprado"><div className="grid gap-3"><label className="grid gap-1 text-sm font-semibold text-[var(--muted)]">Precio real<Input type="number" min="0" step="0.01" value={precio} onChange={(e) => setPrecio(Number(e.target.value))}/></label><label className="grid gap-1 text-sm font-semibold text-[var(--muted)]">Tienda<Select value={tienda} onChange={(e) => setTienda(e.target.value)}><option value="">Selecciona una tienda</option>{SUPERMERCADOS.map((s) => <option key={s}>{s}</option>)}</Select></label><Button onClick={comprar}>Confirmar compra</Button></div></Modal></article>;
}
