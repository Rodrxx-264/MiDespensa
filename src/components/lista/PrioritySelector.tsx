...
export function PrioritySelector({ value = "important", onChange }: { value?: ProductPriority | null; onChange: (value: ProductPriority) => void }) {
  return <div className="grid grid-cols-3 gap-2" role="radiogroup" aria-label="Prioridad del producto">{OPTIONS.map((option) => <button key={option.value} type="button" role="radio" aria-checked={value === option.value} onClick={() => onChange(option.value)} className={`touch rounded-[16px] border px-2 py-2 text-sm font-black ${value === option.value ? "border-tinta bg-tinta text-crema" : "border-tinta/10 bg-white/65 text-tinta/65"}`}>{option.label}</button>)}</div>;
}
