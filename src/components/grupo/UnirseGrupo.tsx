...
export function UnirseGrupo() {
  ...
  return <section className="surface mx-auto max-w-lg rounded-[32px] p-6"><p className="eyebrow">Invitación</p><h1 className="mt-2 text-4xl font-black tracking-[-.06em]">Unirse a un grupo</h1><div className="mt-5 flex flex-col gap-2 sm:flex-row"><Input value={codigo} onChange={(e) => setCodigo(e.target.value)} placeholder="Código QR"/><Button onClick={() => setCodigo(codigo)}>Buscar</Button></div>{grupo && <div className="mt-5 rounded-2xl bg-white/55 p-4"><p className="text-sm text-tinta/60">Grupo encontrado:</p><h2 className="text-xl font-black tracking-[-.04em]">{grupo.nombre}</h2>{perfil?.grupo_id ? <p className="mt-2 text-red-700">Ya perteneces a un grupo. Debes salir primero.</p> : <Button className="mt-3" onClick={unirse}>Unirme</Button>}</div>}</section>;
}
