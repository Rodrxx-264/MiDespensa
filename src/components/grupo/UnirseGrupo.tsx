...
export function UnirseGrupo() {
  ...
  return <section className="surface mx-auto max-w-lg rounded-[32px] p-6"><p className="eyebrow">Invitación</p><h1 className="mt-2 text-4xl font-black tracking-[-.06em]">Unirse a un grupo</h1><p className="mt-2 text-sm text-tinta/60">Escanea el QR o escribe el código corto de 6 caracteres.</p><div className="mt-5 grid gap-2"><Button onClick={() => setScanOpen((v) => !v)}>{scanOpen ? "Cerrar cámara" : "Escanear QR"}</Button>{scanOpen && <div className="overflow-hidden rounded-[24px] bg-tinta"><video ref={videoRef} autoPlay playsInline muted className="aspect-square w-full object-cover" aria-label="Vista de cámara para escanear QR"/></div>}{scanError && <p className="rounded-[18px] bg-white/65 p-3 text-sm text-[#8b3f2f]">{scanError}</p>}<div className="flex flex-col gap-2 sm:flex-row"><Input className="font-mono text-lg uppercase tracking-[.18em]" value={codigo} onChange={(e) => setCodigo(normalizeGroupCode(e.target.value))} placeholder="ABC123" maxLength={36}/><Button variant="secondary" onClick={() => buscarGrupo()}>Buscar</Button></div><Button disabled={!codigo} onClick={unirse}>Unirme con este código</Button></div>{error && <p className="mt-3 rounded-[18px] bg-white/65 p-3 text-sm text-[#8b3f2f]">{error}</p>}{grupo && <div className="mt-5 rounded-2xl bg-white/55 p-4"><p className="text-sm text-tinta/60">Grupo encontrado:</p><h2 className="text-xl font-black tracking-[-.04em]">{grupo.nombre}</h2>{perfil?.grupo_id ? <p className="mt-2 text-red-700">Ya perteneces a un grupo. Debes salir primero.</p> : <Button className="mt-3" onClick={unirse}>Unirme</Button>}</div>}</section>;
}
