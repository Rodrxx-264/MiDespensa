...
export function useListaActiva(grupoId?: string | null) {
  ...
  useEffect(() => {
    if (!lista?.id || isLocalMode()) return;
  ...
  async function actualizar(id: string, data: Partial<Producto>) { if (isLocalMode()) { const state = readLocalState(); const p = state.productos.find((x) => x.id === id); if (p) Object.assign(p, data, { updated_at: new Date().toISOString() }); writeLocalState(state); if (p) recalcLocalList(p.lista_id); await cargar(); return; } await supabase.from("productos").update(data).eq("id", id); await cargar(); }
  async function eliminar(id: string) { if (isLocalMode()) { const state = readLocalState(); const p = state.productos.find((x) => x.id === id); state.productos = state.productos.filter((x) => x.id !== id); writeLocalState(state); if (p) recalcLocalList(p.lista_id); await cargar(); return; } await supabase.from("productos").delete().eq("id", id); await cargar(); }
  async function cerrar(nombre: string) { if (!lista) return; if (isLocalMode()) { const state = readLocalState(); const l = state.listas.find((x) => x.id === lista.id); if (l) Object.assign(l, { estado: "completada", nombre, cerrada_por: state.perfil.id, completada_at: new Date().toISOString() }); writeLocalState(state); setLista(null); await cargar(); return; } const { data: { user } } = await supabase.auth.getUser(); await supabase.from("listas").update({ estado: "completada", nombre, cerrada_por: user?.id, completada_at: new Date().toISOString() }).eq("id", lista.id); setLista(null); await cargar(); }
