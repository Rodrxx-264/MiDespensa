...
export function useGrupo() {
  ...
  async function unirse(codigo: string) {
    ...
    const { data: g, error } = await supabase.from("grupos").select("*").eq("codigo_qr", normalizeGroupCode(codigo)).single(); if (error) throw error;
  ...
  async function regenerarQR() { if (!grupo) return; const { data: { user } } = await supabase.auth.getUser(); if (!user && isLocalMode()) { const state = readLocalState(); if (state.grupo) state.grupo.codigo_qr = createGroupCode(); writeLocalState(state); await cargar(); return; } await supabase.from("grupos").update({ codigo_qr: createGroupCode() }).eq("id", grupo.id); await cargar(); }
