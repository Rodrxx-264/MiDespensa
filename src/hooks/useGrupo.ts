...
export function useGrupo() {
  ...
  async function regenerarQR() { if (!grupo) return; await supabase.from("grupos").update({ codigo_qr: uuid() }).eq("id", grupo.id); await cargar(); }
  async function salir(id = perfil?.id) { if (!id) return; await supabase.from("perfiles").update({ grupo_id: null }).eq("id", id); await cargar(); }
